...
/// Identificador estável da acção personalizada do botão «Direto» (Android).
const String kBibleFmNotificationLiveAction = 'bible_fm_live';

/// Rótulo do botão «Direto» na notificação (manter alinhado com a app, se alterar).
const String kBibleFmNotificationLiveControlLabel = 'Direto';

/// Liga o toque em «Direto» na notificação à app, com **um único [owner]** activo.
///
/// [register] no `init` e [unregister] no `dispose` com o mesmo objecto (p.ex. o
/// [StateNotifier] do leitor).
abstract final class BibleFmMediaNotificationLiveBridge {
  static Object? _owner;
  static void Function()? _onLiveTap;

  static void register(Object owner, void Function() onLiveTap) {
    assert(
      _owner == null || identical(_owner, owner),
      'BibleFmMediaNotificationLiveBridge: já existe outro owner; chame unregister primeiro.',
    );
    _owner = owner;
    _onLiveTap = onLiveTap;
  }
  ...
  /// Chamado por [AudioHandler.customAction] quando o utilizador toca «Direto».
  static void notifyLiveTapFromNotification() => _onLiveTap?.call();
}

/// Até 3 índices: típico limite do MediaStyle compacto em Android.
List<int> _androidCompactMediaControlIndices(List<MediaControl> controls) {
  final out = <int>[];
  for (var i = 0; i < controls.length; i++) {
    if (controls[i].action == MediaAction.stop) continue;
    out.add(i);
...
class JustAudioBackground {
  ...
  /// Actualiza metadados na notificação e na sessão de média **sem** recarregar o stream.
  ///
  /// Chamar a partir do isolate principal após [init]. O [MediaItem.id] tem de coincidir
  /// com o item actual na fila (o mesmo usado na tag do [AudioSource]).
  static Future<void> updateNowPlayingMediaItem(MediaItem mediaItem) async {
...
class _PlayerAudioHandler extends BaseAudioHandler
  ...
  Future<dynamic> customAction(
    ...
    if (name == kBibleFmNotificationLiveAction) {
      BibleFmMediaNotificationLiveBridge.notifyLiveTapFromNotification();
      return null;
    }
  ...
  void _broadcastState() {
    final controls = <MediaControl>[
      ...
      MediaControl.custom(
        androidIcon: 'drawable/ic_biblefm_live',
        label: kBibleFmNotificationLiveControlLabel,
        name: kBibleFmNotificationLiveAction,
      ),
      MediaControl.stop,
